...
# Detectar a codificação e validar todos os bytes evita reparar caracteres silenciosamente.
...
# Ler todos os registros com largura conferida preserva campos e cabeçalhos originais.
def ler_base(caminho: Path) -> tuple[pd.DataFrame, dict]:
    ...
    try:
        ...
        possibilidades = []
        for candidato in ";,\t|":
            cabecalho_candidato = next(csv.reader(io.StringIO(texto, newline=""), delimiter=candidato, strict=True), [])
            if len(cabecalho_candidato) > 1:
                possibilidades.append((len(cabecalho_candidato), candidato))
...
# Examinar formatos e limites em séries auxiliares permite diagnosticar sem tratar.
def diagnosticar_datas(serie: pd.Series, nome: str) -> dict:
    preenchidos = serie[serie.ne("") & serie.notna()]
    # Reunir os formatos aceitos para reconhecer candidatos sem converter a base.
    padroes = []
    for padrao, formato in FORMATOS_DATA:
        padroes.append(padrao)
    padrao_data = "(?:" + "|".join(padroes) + ")"
...
# Inferir o tipo pela escrita dos valores descreve a coluna sem convertê-la.
...
# Inspecionar um ano inteiro produz contagens e evidências para comparação.
def inspecionar_ano(ano: int) -> tuple[dict, list[dict]]:
    ...
    quadro, metadados = ler_base(caminho)
    detalhes = []
    ...
    # Separar os campos de data e os vazios para tornar o resumo legível.
    # Compra é a referência preferida; os demais limites continuam documentados.
    campos_data = []
    nomes_campos_data = []
    colunas_vazias = []
    limites_datas = []
    for detalhe in detalhes:
        if detalhe["datas_validas"]:
            campos_data.append(detalhe)
            nomes_campos_data.append(detalhe["nome_coluna_original"])
        if detalhe["completamente_vazia"]:
            colunas_vazias.append(detalhe["nome_coluna_original"])
        if detalhe["possivel_campo_data"]:
            limite = {}
            for campo in ("nome_coluna_original", "data_minima", "data_maxima",
                          "datas_validas", "valores_nao_vazios_invalidos_como_data"):
                limite[campo] = detalhe[campo]
            limites_datas.append(limite)
    referencia = {}
    if campos_data:
        referencia = campos_data[0]
    for detalhe in campos_data:
        if detalhe["nome_coluna_original"] == "compra":
            referencia = detalhe
            break
    resumo = {
        "ano": ano,
        **metadados,
        "numero_registros": len(quadro),
        "numero_colunas": len(quadro.columns),
        ...
        "colunas_completamente_vazias": texto_json(colunas_vazias),
        "campos_data_validos": texto_json(nomes_campos_data),
        "campo_data_referencia": referencia.get("nome_coluna_original", ""),
        "data_minima": referencia.get("data_minima", ""),
        "data_maxima": referencia.get("data_maxima", ""),
        "limites_por_campo_data": texto_json(limites_datas),
    ...
    if sha256(caminho) != metadados["sha256_csv"]:
        raise RuntimeError(f"O arquivo mudou durante a inspecao: {caminho}")
    print(f"[{ano}] OK: {len(quadro):,} registros; {len(quadro.columns)} colunas; {metadados['encoding']}; separador {metadados['separador']!r}.", flush=True)
    return resumo, detalhes


def comparar(resumos: list[dict], detalhes: list[dict]) -> tuple[list[dict], list[dict], int]:
    # Comparar os cabeçalhos sem alterar a ordem das colunas nas bases.
    # União mostra todas as colunas; interseção mostra as presentes em todos os anos.
    cabecalhos = {}
    for resumo in resumos:
        cabecalhos[resumo["ano"]] = json.loads(resumo["nomes_colunas_originais"])
    todas_colunas = set()
    comuns = set(next(iter(cabecalhos.values())))
    for nomes in cabecalhos.values():
        todas_colunas.update(nomes)
        comuns.intersection_update(nomes)
    uniao = sorted(todas_colunas)
    matriz = []
    discrepancias = []
    ...
    for nome in uniao:
        presentes = []
        linha_matriz = {"nome_coluna_original": nome}
        for ano in ANOS:
            presente = nome in cabecalhos[ano]
            linha_matriz[str(ano)] = presente
            if presente:
                presentes.append(ano)
        linha_matriz["quantidade_anos_presente"] = len(presentes)
        linha_matriz["comum_a_todos"] = nome in comuns
    ...
    # Agrupar diagnósticos por coluna permite comparar tipos entre os anos.
    por_nome = {}
    for detalhe in detalhes:
        nome_coluna = detalhe["nome_coluna_original"]
        if nome_coluna not in por_nome:
            por_nome[nome_coluna] = []
        por_nome[nome_coluna].append(detalhe)
        if detalhe["completamente_vazia"]:
    ...
    for nome, itens in por_nome.items():
        ...
        for detalhe in itens:
            tipos.add(detalhe["tipo_inferido"])
            anos_coluna.add(detalhe["ano"])
            evidencia.append({
                "ano": detalhe["ano"],
                "posicao": detalhe["posicao_coluna"],
                "tipo": detalhe["tipo_inferido"],
            })
        if len(tipos) > 1:
            registrar("diferenca_tipo_inferido", sorted(anos_coluna), nome, texto_json(evidencia))
    ...
    for atributo in ("encoding", "separador"):
        ...
        valores_por_ano = {}
        for resumo in resumos:
            valores_distintos.add(resumo[atributo])
            valores_por_ano[resumo["ano"]] = resumo[atributo]
        if len(valores_distintos) > 1:
            registrar(f"diferenca_{atributo}", ANOS, "", texto_json(valores_por_ano))
    for resumo in resumos:
    ...
    parcial = next(resumo for resumo in resumos if resumo["ano"] == 2026)
